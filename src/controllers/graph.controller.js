...
export class GraphController {
  ...
  async getAllGraphs(_, res) {
    try {
      const graphs = await Graph.find().populate('doctor');
  ...
  async getGraphById(req, res) {
    try {
      const graph = await GraphController.findById(res, req.params.id);
      return res.status(200).json({
        statusCode: 200,
        message: 'success',
        data: graph,
  ...
  async updateGraphById(req, res) {
    try {
      ...
      const graph = await GraphController.findGraphById(res, id);
